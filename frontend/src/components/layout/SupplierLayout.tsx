...
export function SupplierLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-[#f7f9f7]">
      ...
      >
        <Link
          to="/fournisseur/tableau-de-bord"
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-white/50 leading-none mt-0.5">{t('sidebar.supplier.role')}</p>
          </div>
        </Link>
        ...
        <div className="px-3 pb-4 border-t border-white/10 pt-4 space-y-2">
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-white/5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{user?.company ?? user?.name}</p>
              <p className="text-xs text-white/50 truncate">{t('sidebar.supplier.role')}</p>
            </div>
          </div>
          <button
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5 pl-3 border-l border-gray-200">
              ...
              <div className="hidden sm:block">
                <p className="text-sm font-medium text-gray-900 leading-none">{user?.company ?? user?.name?.split(' ')[0]}</p>
                <p className="text-xs text-gray-400 mt-0.5">{t('sidebar.supplier.role')}</p>
              </div>
            </div>
          </div>
        </header>
