...
import { useAdminDiagnostics } from '../hooks/useAdmin';
import type { DiagnosticHistoryItem } from '@/features/diagnostic/types';

export function AdminDiagnosticsPage() {
  const [search, setSearch] = useState('');
  const { data: diagnostics = [], isLoading, isError } = useAdminDiagnostics();
  const [detailsDiag, setDetailsDiag] = useState<DiagnosticHistoryItem | null>(null);
    ...
    <div className="space-y-6">
      ...
      <Card padding="none" className="overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-red-600">Impossible de charger les diagnostics.</div>
        ) : (
